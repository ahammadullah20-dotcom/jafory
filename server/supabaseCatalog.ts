...
const mapProduct = (row: any, categorySlug?: string) => ({ id: String(row.id), categoryId: String(row.category_id), slug: row.slug, nameEn: row.name_en, nameAr: row.name_ar, nameBn: row.name_bn, shortDescriptionEn: row.short_description_en, shortDescriptionAr: row.short_description_ar, shortDescriptionBn: row.short_description_bn, descriptionEn: row.description_en, descriptionAr: row.description_ar, descriptionBn: row.description_bn, imageUrl: productImage(row, categorySlug), badgeEn: row.badge_en, badgeAr: row.badge_ar, badgeBn: row.badge_bn, isFeatured: row.is_featured ? 1 : 0, isActive: row.is_active ? 1 : 0 });
...
export async function supabaseHome() {
  ...
  const [catRows, slideRows, featuredRows, socialRows, settingRows] = await Promise.all([
  ...
  return { categories: cats, slides: slideRows.map(mapSlide), featuredProducts: publicFeatured.map(row => ({ product: mapProduct(row, categorySlugById.get(String(row.category_id))), category: cats.find(c => c.id === String(row.category_id)) ?? null })), socialLinks: socialRows.map(mapSocial), settings: settingRows.map(mapSetting) };
}
...
export async function supabaseCategory(slug: string) {
  ...
  return { category: mapCategory(categoryRow), products: (productRows ?? []).filter(row => canonicalProductSlugs.has(row.slug)).map(row => mapProduct(row, categoryRow.slug)) };
}
...
export async function supabaseSearch(query: string) {
  ...
  return (data ?? []).filter(row => canonicalProductSlugs.has(row.slug)).map(row => { const category = categoryMap.get(String(row.category_id)) ?? { id: "", slug: "", nameEn: "", nameAr: "", nameBn: "", descriptionEn: "", descriptionAr: "", descriptionBn: "", imageUrl: null, accentColor: "teal" }; return { row, category }; }).filter(({ row, category }) => isMinimumSearchMatch(tokens, [row.name_en, row.name_ar, row.name_bn, row.slug, category.slug, category.nameEn, category.nameAr, category.nameBn])).map(({ row, category }) => ({ product: mapProduct(row, category.slug), category }));
}
...
export async function supabaseProduct(slug: string) {
  ...
  return { product: mapProduct(productRow, category.slug), category, media: parsePublicMedia(mediaSetting.data?.setting_value), specifications: (specs.data ?? []).map(mapSpec), affiliateLinks: (links.data ?? []).map(mapLink), reviews, rating: { count: reviews.length, average: reviews.length ? reviews.reduce((sum: number, row: any) => sum + row.rating, 0) / reviews.length : 0 } };
}
...
export async function supabaseCompare(ids: Array<string | number>) {
  ...
  const result = await Promise.all(rows.map(async row => { const detail = await supabaseProduct(row.slug); return detail ? { ...detail, product: mapProduct(row) } : null; }));
